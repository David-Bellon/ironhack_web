...
@app.route("/RacePace", methods=["POST"])
def race_pace():
    receive = request.json
    session = fastf1.get_session(2023, receive["race"], "r")
    session.load()
    laps = session.laps
    fig, ax = plt.subplots(figsize=(15, 10))
    ax.set_xlabel('Laps')
    ax.set_ylabel('Lap Time')
    scout = [receive["driver1"], receive["driver2"], receive["driver3"]]
    for driver in scout:
        myDriver = laps.loc[laps["Driver"] == driver]
        myDriverLaps = myDriver["LapTime"]
        ax.plot(list(range(1, len(myDriverLaps) + 1)), myDriverLaps, label=driver)
        ax.legend()
    plt.savefig("temp.png")
    return send_file("temp.png")
